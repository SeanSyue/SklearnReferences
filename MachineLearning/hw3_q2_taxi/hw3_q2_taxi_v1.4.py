# Source:
# https://github.com/ffyu/Kaggle-Taxi-Travel-Time-Prediction/blob/master/Submission.py
...
class MakeOutputDirectory:
    ...
    def get_dir_name(self, print_dir):
        ...
        elif type(self.suffix) == tuple:
            for i in range(0, len(self.suffix)):
                self.prefix += "_"+str(self.suffix[i])
        else:
            try:
                raise TypeError('LAYERS must be tuple or int type')
            except TypeError:
                print('OOOOOOOOOOOOOOOOOOOOOOOOOOOOOOOOOOps')
                raise
...
#             df_hour = pd.read_csv(self.folder+file1, header=0)
...
#             df_weekday = pd.read_csv(self.folder+file2, header=0)
...
#             df_month = pd.read_csv(self.folder+file3, header=0)
...
#             df_driver = pd.read_csv(self.folder+file4, header=0)
#             # Join by driver id
...
#             df_stand = pd.read_csv(self.folder+file5, header=0)
...
#             df_caller = pd.read_csv(self.folder+file6, header=0)
...
#
#         df_Test.sort_values(['index'], ascending=1, inplace=True)
#         df_Test.reset_index(inplace=True)
...
class ScalableRandomForest:
    ...
    def __init__(self, num_trees, q, layers, folder, folder_out, train_file, test_file, report_file):
    ...
    def train_predict_by_partition(self):
        ...
        for i in range(self.num_trees):
            ...
            df_X = df[['Call_Type__A', 'Call_Type__B', 'Call_Type__C',
                        ...
                        'Stand_TT', 'Stand_TL', 'Stand_TC', 'Stand_TS',
                        'Caller_TT', 'Caller_TL', 'Caller_TC', 'Caller_TS',
                        'Start_Speed', 'End_Speed', 'Avg_Speed', 'Current_Snapshots']]
            ...
            clf = MLPRegressor(activation='logistic', hidden_layer_sizes=self.layers, max_iter=2000)
            clf = clf.fit(X, y)
            ...
            df_result.to_csv(self.folder_out+'submission_train{}.csv'.format(i), index=False)
        ...
        print("Train duration:{}s".format(duration))
    ...
    def ensemble_all_partitions(self):
        ...
        for i in range(num_files):
            ...
            dfs.append(pd.read_csv(self.folder_out+prediction_files[i]))
        ...
        y_test = np.mean(Travel_Time, axis=1)
        result = np.c_[IDs, np.around(y_test).astype(int)]
        df_result = pd.DataFrame(result, columns=['TRIP_ID', 'TRAVEL_TIME'])
        df_result.to_csv(self.folder_out+'submission_final.csv', index=False)
    ...
    def do_evaluation(self):
        ...
        df_test = pd.read_csv(self.folder + self.test_file)
        actual_time = df_test['Current_Snapshots'].map(lambda x: (x - 1) * 15)
        df_submission = pd.read_csv(self.folder_out+'submission_final.csv')
        pred_time = df_submission['TRAVEL_TIME'].values

        result = np.c_[pred_time, actual_time]
        df_comparison = pd.DataFrame(result, columns=['pred_time', 'actual_time'])
        df_comparison = df_comparison[df_comparison['actual_time'] != 0]
        df_comparison['error'] = df_comparison.pred_time - df_comparison.actual_time
        df_comparison['log_sqr_error'] = np.square(np.log(df_comparison.pred_time + 1) -
                                                   np.log(df_comparison.actual_time + 1))
        print(df_comparison)
        df_comparison.to_csv(f'D:/hw3_q2_taxi/comparison{self.layers}.csv')

        rmsle = np.sqrt(np.mean(df_comparison['log_sqr_error']))
        print("RMSLE:", rmsle)
        self.rmsle = rmsle

    def report_writer(self):
        with open(self.report_file, 'a') as text_file:
            print(f"folder: {self.folder_out}\n"
...
def main():
    FOLDER = 'D:/hw3_q2_taxi/Taxi2/'
    PARENT_PATH = 'D:/hw3_q2_taxi/'
    for i in range(1, 5):
        for j in range(1, 31):
            LAYERS = (i, j)

            make_dir(PARENT_PATH)  # Check for parent path. If it's not exist, create one.
            dir_manager = MakeOutputDirectory(prefix=PARENT_PATH + 'hw3_q2_taxi', suffix=LAYERS)
            ...
            srf = ScalableRandomForest(num_trees=1000, q=1, layers=LAYERS, folder=FOLDER, folder_out=FOLDER_OUT,
                                       train_file='train_final.csv', test_file='test_final.csv',
                                       report_file=PARENT_PATH + 'report.txt')
