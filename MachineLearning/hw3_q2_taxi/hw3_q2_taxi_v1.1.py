...
class MakeDirectory:
    ...
    def __init__(self, prefix, suffix_tuple):
        self.prefix = prefix
        self.suffix_tuple = suffix_tuple
        self.folder_out = None

    def get_dir_name(self, print_dir):
        prefix = self.prefix
        for i in range(0, len(self.suffix_tuple)):
            prefix += "_" + str(self.suffix_tuple[i])
            folder_out = prefix + "/"
...
class ScalableRandomForest:
    ...
    def __init__(self, num_trees, q, layers, folder, folder_out, train_file, test_file):
...
def main():
    ...
    dir_manager = MakeDirectory(prefix=PARENT_PATH+'hw3_q2_taxi', suffix_tuple=LAYERS)
